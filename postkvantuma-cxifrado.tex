...
\begin{frame}
  ...
  \begin{description}
  \item<2->[konfidenteco] \only<5->{\color<6->{green} neniu necelata povas legi la datumojn}
  \item<3->[fidindeco] \only<5->{\color<6->{green} pruveblo de la deveno kaj la aŭtenteco}
  \item<4->[disponeblo] \only<5->{\color<6->{red} datumoj estas disponebla kiam bezonataj}
  \end{description}
\end{frame}
...
\begin{frame}
  ...
  \item $f_c(K, S) = M$
  \item $f_M(M, S) = K$
  \item $S$ estas la ŝlosilo por ĉifri kaj malĉifri
  \end{itemize}
...
\begin{frame}
  ...
  \begin{tikzpicture}[node distance=0.5em,align=left]
    ...
    \node [below=of anjo] (priv) {{\uncover<2->{$S_{priv}$}}};
    \node [below=of priv] (pub_a) {{\uncover<3->{$S_{pub} = P(S_{priv})$}}};
    \node (pub_b) at (pub_a -| boĉjo) {{\uncover<5->{$S_{pub}$}}};
    \node (pub) at ($(pub_a)!0.5!(pub_b)$) {{\uncover<4->{$S_{pub}$}}};
    ...
    \node [below=of klar] (cf_b) {{\uncover<7->{$M = f_C(K, S_{pub})$}}};

    \node (cf_a) at (cf_b -| anjo) {{\uncover<8->{$M$}}};
    \uncover<8->{\draw[->] (cf_b) -- (cf_a);}

    \node [below=of cf_a] (mcf) {{\uncover<9->{$K = f_M(M, S_{priv})$}}};
...
\begin{frame}
  ...
  \begin{tikzpicture}[>=stealth,shorten >=1pt]
    \begin{scope}[every node/.style=draw]
      \node [draw] (input) {
        $\invisible<3->{\left|\color{red}{1}\right> + \left|\color{blue}{2}\right> +}
        \left|\color{magenta}{3}\right>
        \invisible<3->{+ \left|\color{green}{4}\right> + \left|\color{cyan}{5}\right>}$
      };
      \node [draw, right=1em of input] (operation) {$+3$};
      \node [draw, right=1em of operation] (output) {
        $
        \invisible<3->{\left|\color{red}{4}\right> + \left|\color{blue}{5}\right> +}
        \left|\color{magenta}{6}\right>
        \invisible<3->{+ \left|\color{green}{7}\right> + \left|\color{cyan}{8}\right>}
        $};
    \end{scope}
    \node [above=0.1em of input] {enigo};
    \node [above=0.1em of operation] {operacio};
...
\begin{frame}[label=shor]
  ...
  \uncover<5->{%
    ...
    \begin{itemize}
    \item $p$ estas para
    \item $d^p \pm 1$ estas divizoro de $N$
    \end{itemize}
    \uncover<6>{
      \begin{beamercolorbox}[center, rounded=true, shadow=true]{notebox}
        Kiel kalkuli $p$?}
...
\begin{frame}
  ...
  \begin{tikzpicture}[>=stealth,shorten >=1pt]
    \node [draw] (input) {
      ...
      \ldots $
    } node [above=1em] {\small ĉiuj eblaj valoroj por $p$};
    \node [draw, below=of input.west] (step1) {$d^x$};
    \node [draw, right=of step1] (potential) {$\left|d^1\right> + \left|d^2\right> + \left|d^3\right> + \left|d^4\right> + \ldots $};
    \node [draw, below=of step1] (mod) {$x\mod N$};
    \node [draw, right=of mod] (mods) {$\left|r_1\right> + \left|r_2\right> + \left|r_3\right> + \left|r_4\right> + \ldots $};
    \path
  ...
  \[ d^p = m_p N + 1 \; ; \; d^i = m_i N + r_i \Longrightarrow d^{i+p} = m_{i+p} N + r_i \]
\end{frame}
...
\begin{frame}
  ...
  \begin{tikzpicture}[>=stealth,shorten >=1pt]
    ...
    \uncover<3->{
      \node [draw, right=of qft] (result) {$1/p$};
      \draw [->] (qft) -- (result);
    }
  \end{tikzpicture}
  \end{figure}
  ...
  \begin{columns}
    \begin{column}{0.4\textwidth}
      \begin{tikzpicture}
        ...
        \node (nine) at (270:2em) {$\lfloor \frac{q}{2} \rceil = 9$};
      \end{tikzpicture}
    \end{column}
